...
if __name__ == "__main__":
    ...
    for dataset_path, index_path in zip(datasets_paths, indexes_paths):
        ...
        logging.info('        Max est.get_depth() :' + str(np.max([est.get_depth() for est in grid.best_estimator_.estimators_])))
        logging.info('        Max est.tree_.max_depth :' + str(np.max([est.tree_.max_depth for est in grid.best_estimator_.estimators_])))
